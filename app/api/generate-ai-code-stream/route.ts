      ...
      try {
        ...
        if (isEdit) {
          ...
          if (manifest) {
            ...
            console.log('[generate-ai-code-stream] Files available for search:', fileContents ? Object.keys(fileContents).length : 0);
...
5. Make the requested change immediately`;
        ...
        const modelProvider = isAnthropic ? anthropic : (isOpenAI ? openai : (isGoogle ? googleGenerativeAI : (isOpenRouter ? openrouter : groq)));
